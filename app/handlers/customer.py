...
async def customer_close_yes(callback: CallbackQuery, db) -> None:
    ...
    if order.get("assigned_executor_id"):
        ...
        await callback.message.answer("Ожидайте подтверждение исполнителя")
    else:
...
async def customer_responses_new(callback: CallbackQuery, db) -> None:
    ...
    if not executor:
        await callback.message.answer("Доступные исполнители закончились")
        await callback.answer()
        return
...
async def customer_responses_liked(callback: CallbackQuery, db) -> None:
    ...
    if not matches:
        await callback.message.answer("Принятых исполнителей нет.")
        await callback.answer()
        return
    lines = []
    buttons = []
...
async def customer_responses_declined(callback: CallbackQuery, db) -> None:
    ...
    if not matches:
        await callback.message.answer("Отказанных исполнителей нет.")
        await callback.answer()
        return
    buttons = []
    lines = []
...
async def customer_change_decision(callback: CallbackQuery, db) -> None:
    ...
    await callback.message.answer("Решение изменено.")
